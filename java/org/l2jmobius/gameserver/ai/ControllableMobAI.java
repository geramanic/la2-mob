...
public class ControllableMobAI extends AttackableAI
{
	...
	{
		if (!LocationUtil.checkIfInRange(MobGroupTable.FOLLOW_RANGE, _actor, getForcedTarget(), true))
		{
	...
	protected void thinkCast()
	{
		...
		{
			if (LocationUtil.checkIfInRange(sk.getCastRange(), _actor, getAttackTarget(), true) && !_actor.isSkillDisabled(sk) && (_actor.getCurrentMp() > _actor.getStat().getMpConsume(sk)))
			{
				_actor.doCast(sk);
				return;
			}
	...
	protected void thinkAttack()
	{
		...
		else
		{
			...
			final double distance = _actor.calculateDistance2D(getAttackTarget());
